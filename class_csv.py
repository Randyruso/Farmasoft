#!/usr/bin/python3
#esto genera una lista con objetos, donde cada objeto es un registro del csv

import csv
def classificar(nombre_archivo):
#    nombre_archivo='metal.csv'
        ...
        for linea in archivo_csv:
            if x == 0:
                ...
                for y in range(5):
                    ...
                    y = y + 1
                x = x + 1
#                registros.append(Csv(cliente = linea[col_cliente].strip(' ').upper(), codigo = linea[col_codigo].strip(' ').upper(), producto = linea[col_producto].strip(' ').upper(), cantidad = linea[col_cantidad].strip(' ').upper(), precio = linea[col_precio].strip(' ').upper()))
            else:
